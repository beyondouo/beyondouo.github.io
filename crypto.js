...
const APIS = [
  ...
  { name: 'MEXC',     price: s => `https://api.mexc.com/api/v3/ticker/24hr?symbol=${s}USDT`, parse: d => { if(!d?.lastPrice)return null; const l=parseFloat(d.lastPrice),o=parseFloat(d.openPrice); return {p:l,h:parseFloat(d.highPrice)||0,l:parseFloat(d.lowPrice)||0,v:parseFloat(d.quoteVolume)||0,c:o?((l-o)/o*100):parseFloat(d.priceChangePercent)||0}; } },
  { name: 'Bitget',   price: s => `https://api.bitget.com/api/v2/spot/market/tickers?symbol=${s}USDT`, parse: d => { const v=d?.data?.[0]; return v ? {p:parseFloat(v.lastPr),h:parseFloat(v.high24h)||0,l:parseFloat(v.low24h)||0,v:parseFloat(v.usdtVolume)||0,c:(parseFloat(v.change24h)||0)*100} : null; } },
